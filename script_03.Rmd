...
- Decide what data is relevant to the problem we want to solve
   - undergraduate enrollment numbers per class for Sept 1 and today
- We should define what we want to know
   - want to know how the enrollment numbers have changed (if changed)!


After we have a model, Temperance suggests we should:
- recognize the limitations of our model 
   - can't know why people are dropping
   - we make no distinction between people who add vs people who drop
...
ugrad_diff <- inner_join(sep_old, sep_new, by = "id") %>% 
  ...
  arrange(diff) %>% 
  slice(1:5, 392:396) %>% 
  mutate(sign = ifelse(diff > 0, "Increase", "Decrease"))

ugrad_diff %>% 
  ggplot(aes(title.x, abs(diff))) +
  geom_col() +
  facet_wrap(~sign) +
  coord_flip() +
  theme_bw()
